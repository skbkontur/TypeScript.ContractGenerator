...
namespace SkbKontur.TypeScript.ContractGenerator.Tests
{
    public class OptionsTests : AllTypeCheckersTestBase
    {
        ...
        {
            var generatedCode = GenerateCode(new TypeScriptGenerationOptions {EnableOptionalProperties = optionalPropertiesEnabled, EnumGenerationMode = EnumGenerationMode.FixedStringsAndDictionary}, CustomTypeGenerator.Null, typeof(SingleNullablePropertyType)).Single();
            var expectedCode = GetExpectedCode($"Options/{expectedFileName}");
            generatedCode.Diff(expectedCode).ShouldBeEmpty();
        }
        ...
        {
            var generatedCode = GenerateCode(new TypeScriptGenerationOptions {EnableExplicitNullability = explicitNullabilityEnabled, EnumGenerationMode = EnumGenerationMode.FixedStringsAndDictionary}, CustomTypeGenerator.Null, typeof(ExplicitNullabilityRootType)).Single();
            var expectedCode = GetExpectedCode($"Options/{expectedFileName}");
            generatedCode.Diff(expectedCode).ShouldBeEmpty();
        }
        ...
        {
            var generatedCode = GenerateCode(new TypeScriptGenerationOptions {UseGlobalNullable = useGlobalNullable, EnumGenerationMode = EnumGenerationMode.FixedStringsAndDictionary}, CustomTypeGenerator.Null, typeof(GlobalNullableRootType)).Single();
            var expectedCode = GetExpectedCode($"Options/{expectedFileName}");
            generatedCode.Diff(expectedCode).ShouldBeEmpty();
        }
        ...
        {
            var options = new TypeScriptGenerationOptions {NullabilityMode = NullabilityMode.NullableReference, EnumGenerationMode = EnumGenerationMode.FixedStringsAndDictionary};
